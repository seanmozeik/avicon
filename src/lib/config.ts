import { SECRETS_SERVICE, CONFIG_KEY } from './secrets.js';
...
let _cache: ViconConfig | null | undefined = undefined;
...
export async function getConfig(): Promise<ViconConfig | null> {
  ...
  // Try Bun.secrets
  try {
    const val = await Bun.secrets.get({ service: SECRETS_SERVICE, name: CONFIG_KEY });
    if (val) {
...
export async function setConfig(config: ViconConfig): Promise<void> {
  try {
    await Bun.secrets.set({ service: SECRETS_SERVICE, name: CONFIG_KEY, value: JSON.stringify(config) });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    if (msg.toLowerCase().includes('libsecret') || msg.toLowerCase().includes('secret service')) {
      throw new Error(
